...
struct studiohdr_t {
    ...
    name: [i8; 64], // The internal name of the model, padding with null bytes.
    // Typically "my_model.mdl" will have an internal name of "my_model"
    data_length: i32, // Data size of MDL file in bytes.

    // A Vec3 is 12 bytes, three 4-byte f32-values in a row.
    eyeposition: Vec3,   // Position of player viewpoint relative to model origin
    illumposition: Vec3, // Position (relative to model origin) used to calculate ambient light contribution and cubemap reflections for the entire model.
    hull_min: Vec3,      // Corner of model hull box with the least X/Y/Z values
    hull_max: Vec3,      // Opposite corner of model hull box
    view_bbmin: Vec3,    // TODO: what's this, how is it different from hull_min/max?
    view_bbmax: Vec3,
    ...
    root_lod: i8, // Preferred rather than clamped

    // 0 means any allowed, N means Lod 0 -> (N-1)
    num_allowed_root_lods: i8,
